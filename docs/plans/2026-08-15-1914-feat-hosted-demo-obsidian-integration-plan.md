...
- **Objective:** Let a fresh Cuecraft installation generate its first complete study experience without configuring an AI provider: cues for up to five sections, a Summary, and a Note Brief.
...
Add a credential-free Cuecraft Demo provider for fresh installations. One explicit user action sends a bounded note bundle to the hosted service and returns up to five section cues, a Summary, and a Note Brief, while existing installations and existing providers retain their current behavior.
...
- **Provide a complete daily study bundle.** (session-settled: user-directed — chosen over cue-only generation: the demo must include section cues, Summary, and Note Brief even if capacity falls to approximately 50 daily users.) Governs R9–R13.
...
flowchart TB
  ...
  Bundle -->|"Success"| Render["Render cues, Summary, and Note Brief"]
...
- R9. One hosted operation shall request cues for the first one to five eligible sections in document order, plus one Summary and one Note Brief.
...
- R11. Summary and Note Brief inputs shall use the same bounded whole-note context and completed section set supplied for the operation rather than only the text of a single section.
- R12. A successful response shall map every cue result, the Summary result, its learning objective when present, and the Note Brief result into the existing Cuecraft study surfaces and cache behavior without modifying the Markdown source.
...
- F1. **Fresh installation generates its first complete bundle**
  ...
  - **Outcome:** A1 sees section cues, Summary, and Note Brief without provider setup or Markdown modification.
  - **Covers:** R1, R2, R5–R17
...
- AE1. **Covers R1, R2, R5–R13.** Given a fresh installation and a note with five eligible sections, when the user accepts the disclosure and invokes generation, then Cuecraft sends one hosted operation and renders five cue results, Summary, and Note Brief without opening Settings.
...
- AE8. **Covers R15.** Given the user has custom cue or Summary instructions, when Cuecraft Demo runs, then the hosted request uses fixed demo instructions and no custom prompt text is transmitted.
...
- Every response displayed as a successful hosted demo includes all requested cues, Summary, and Note Brief in the existing study surfaces.
...
- The current `generateNote` behavior in `src/generator.ts` remains the reference for ordered section cues followed by Summary and Note Brief artifacts, except for the hosted five-section and explicit-action bounds.
...
- `src/settings.ts` defines the current Ollama default, provider settings, section concurrency, automatic Summary preference, and review-surface preferences.
...
- `src/cue-provider.ts` defines the existing cue, batched-cue, Summary, and optional Note Brief runtime capabilities.
- `src/generator.ts` defines ordered section generation, bounded context, Summary generation, optional Note Brief generation, cancellation, and partial-failure behavior.
