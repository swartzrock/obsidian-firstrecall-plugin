...
| Field | Value |
|---|---|
| Objective | Move cloud provider API keys out of Obsidian `data.json` and into Obsidian `app.secretStorage`, while keeping BYOK persistence-free and Electron-free. |
| Authority | User-confirmed direction on 2026-06-29: CueCraft should use Obsidian `SecretStorage`; BYOK should not be tied to Electron; cloud keys must not be stored in `data.json`. |
...
| Stop conditions | Stop if Obsidian `app.secretStorage` is unavailable; stop if migration cannot save a secret without preserving the old plaintext settings. |
...
CueCraft should stop treating cloud API keys as ordinary BYOK settings. `data.json` should keep provider choice, model selection, fetched model metadata, verification snapshots, and non-secret local provider values, while cloud API keys live in Obsidian `app.secretStorage`.
...
Obsidian `SecretStorage` is the best fit because it is the host application's secret API, avoids native binary distribution work, and keeps CueCraft out of Electron internals. The important guardrail is app compatibility: `app.secretStorage` is available in Obsidian 1.11.4 and newer, so older runtimes must fail closed for cloud keys.
...
- R2. CueCraft stores cloud provider API keys in Obsidian `app.secretStorage` under CueCraft-owned secret IDs.
- R3. CueCraft refuses to save, migrate, read, or use cloud API keys when Obsidian `app.secretStorage` is unavailable.
- R4. Existing credentials are not overwritten when secret storage or migration fails.
...
- R8. Existing plaintext cloud API keys migrate into Obsidian secret storage before CueCraft removes them from persisted settings.
...
- R16. Tests cover migration success and failure, no plaintext persistence, unavailable secret storage refusal, runtime key resolution, saved-key UI state, and Ollama/CLI regression behavior.
- R17. Documentation describes CueCraft's secure-storage model and BYOK's caller-owned persistence boundary.

### Acceptance Examples

- AE1. Given `data.json` contains an existing OpenAI key, when CueCraft loads and `app.secretStorage` is available, then the key is saved to secret storage and the next saved `data.json` contains no OpenAI key value.
- AE2. Given migration cannot save to secret storage, when CueCraft loads settings with a legacy plaintext key, then the key remains in settings and CueCraft warns that secure migration is incomplete.
- AE3. Given `app.secretStorage` is unavailable, when a user tries to save a cloud API key, then CueCraft refuses the save and explains that Obsidian 1.11.4 or newer is required.
...
- AE5. Given a saved cloud key and selected model, when generation starts, then CueCraft reads the key from secret storage in memory, creates the BYOK runtime, and does not write the plaintext key back to settings.
...
- CueCraft-owned credential store using Obsidian `app.secretStorage`.
- Obsidian minimum app version update to 1.11.4.
...
- Cross-device credential sync or backup UX for stored secrets.
...
- KTD1. Use Obsidian `app.secretStorage` in CueCraft, not Electron `safeStorage` or `@napi-rs/keyring`. Distribution stays focused on bundled JavaScript, manifest, and styles rather than native binaries or Electron internals.
- KTD2. Store credentials under CueCraft-owned secret IDs. `data.json` keeps only non-secret provider settings and metadata, satisfying the user's requirement that cloud keys are not stored there.
- KTD3. Raise or enforce the Obsidian runtime requirement for `SecretStorage`, which is available starting in Obsidian 1.11.4.
- KTD4. Treat missing `app.secretStorage` as unavailable for cloud secrets. A feature that silently stores secrets in normal plugin data is worse than an explicit setup block.
...
- KTD8. Make migration two-phase. Write and read-check the secret first, then remove plaintext from settings and save `data.json`; on failure, keep the old value and notify the user.
...
flowchart TB
  ...
  CredentialStore --> SecretStorage["Obsidian app.secretStorage"]
...
sequenceDiagram
  ...
  alt cloud plaintext key exists
    Load->>Store: save key to secret storage
...
- Settings persistence changes from one JSON-only store to JSON metadata plus Obsidian secret storage.
...
- Tests must mock the credential store and Obsidian `SecretStorage` instead of importing Electron directly.
...
| Use `@napi-rs/keyring` in CueCraft | Rejected for now | It matches the upstream `cc-byok` model, but native binary distribution is unnecessary while Obsidian provides `SecretStorage`. |
...
| Use Electron `safeStorage` directly | Rejected | Obsidian's renderer Electron module does not reliably expose `safeStorage`, while `app.secretStorage` is the supported host API. |
...
| Add command/env secret providers now | Deferred | It is a useful future fallback, but it widens UX and threat-model scope beyond the confirmed `SecretStorage` plan. |

### Risks & Dependencies

- **Obsidian version compatibility:** Older Obsidian versions do not expose `app.secretStorage`. Mitigation: require or guard for Obsidian 1.11.4 and keep local providers usable.
- **Migration loss:** Removing plaintext before secret storage is durable would lose keys. Mitigation: two-phase migration with read-check and no deletion on failure.
...
- **Secret payload corruption:** Secret values can be malformed or unreadable. Mitigation: fail closed for cloud providers, keep old entries on write failure, and surface repair actions.
...
- `manifest.json` already marks the plugin desktop-only and should now require Obsidian 1.11.4 or newer for `app.secretStorage`.
- Obsidian `node_modules/obsidian/obsidian.d.ts` documents `App.secretStorage` and `SecretStorage` as `@since 1.11.4`.
- [Obsidian `SecretStorage` API](https://docs.obsidian.md/Reference/TypeScript+API/SecretStorage) documents the secret storage methods used by CueCraft.
...
- **Goal:** Introduce a CueCraft-owned credential storage boundary backed by Obsidian `app.secretStorage`.
- **Requirements:** R1, R2, R3, R4, R16
- **Dependencies:** None
- **Files:** `src/secure-credential-store.ts`, `tests/secure-credential-store.test.ts`
- **Approach:** Define a small async credential-store interface with operations for capability check, save, read, clear, and presence. The implementation should store versioned payloads through Obsidian `SecretStorage` and reject cloud secret operations when `app.secretStorage` is unavailable.
...
- **Test scenarios:**
  - Saving a cloud key with an available fake `SecretStorage` writes to a CueCraft-owned secret ID.
  - Reading a saved key returns the original plaintext without exposing it through settings metadata.
  - Clearing a saved key removes only that provider entry and leaves other provider entries intact.
  - When `SecretStorage` is unavailable, cloud save/read reports unavailable.
  - When a stored secret payload is corrupt, reads fail closed with an actionable error.
  - When a write fails after an old secret exists, the old secret remains available.
- **Verification:** Credential storage works through fakes without importing Electron in tests, and unavailable storage states are represented as first-class results instead of swallowed exceptions.
...
- **Approach:** During plugin data load, detect cloud plaintext values from legacy flat settings and nested BYOK settings. For each cloud provider, write and read-check the secret before clearing the plaintext and saving sanitized settings. If any provider fails migration, keep its plaintext field and record a warning state for settings to render.
...
- **Approach:** Update docs to say CueCraft stores non-secret BYOK settings in `data.json` and cloud API keys in Obsidian secret storage. Keep the old AI settings plan as historical context, but add a supersession note so future implementers do not follow its storage wording.
...
- Cloud API keys are stored only in Obsidian secret storage after successful save or migration.
