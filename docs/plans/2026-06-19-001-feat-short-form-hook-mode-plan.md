...
Implement Short-Form Hook Mode as an opt-in, high-energy Study Mode surface that turns cached CueCraft questions into vertical, title-card-style recall prompts with staged keyword beats and a final summary card. The rollout should start with display-only transformations over existing cache fields, then add interaction, persistence, and evaluation instrumentation only after the prototype proves useful.

## Problem Frame

The origin brainstorm frames Short-Form Hook Mode as a sprint mode, not a replacement for calm Cornell reading. The mode should make active-recall questions more memorable while preserving answer intent, sequence support terms as revealable beats, and convert the note summary into a final title card. It must preserve CueCraft's non-destructive promise, keep text as selectable HTML, and use existing generated fields before changing provider contracts.
...
- R4. Keyword supports must appear as staged beats that can be hidden, softened, tapped, or revealed without making recall passive.
...
- Tappable or revealable keyword beats.
...
- KTD1. Build a separate presentation model before rendering: add pure helpers near `src/cornell.ts` so hook-card wording, support beat limiting, empty-state behavior, and summary-card selection can be tested without Obsidian DOM.
...
flowchart TB
  ...
  CornellView --> Beats[staged keyword beats]
...
The design direction is documented as a self-contained HTML mockup at `docs/designs/short-form-hook-mode.html`, with a screenshot-style SVG artifact at `docs/screenshots/short-form-hook-mode-design.svg`. The visual thesis is warm, safe learning energy inside Obsidian constraints: a left-margin cue rail, one compact hook card per note section, staged support beats, a final summary card, and a single explainer graph that defines the rail labels and states.
...
- **Card anatomy:** Each note section gets a compact card aligned beside the Markdown body. A card contains a section marker, a transformed hook title, staged support beats, and optional actions for revealing a beat versus revealing the answer. The original question remains in the presentation model for fidelity checks and accessibility metadata.
- **Support beat states:** Revealed beats use solid high-contrast chips; unrevealed beats use dashed or softened chips with explicit tap copy. This keeps supports visible as a sequence without turning them into passive hints too early.
- **Final summary card:** The note summary becomes the final rail card after the section cards, with the learning objective as optional quiet supporting text. This card should feel like synthesis, not another hint.
- **Theme translation:** Production CSS should express this through Obsidian variables first (`--background-*`, `--text-*`, `--interactive-accent`) and only use custom fallbacks for the demo page. The default Hook Mode palette should stay warm and learning-safe: parchment backgrounds, amber/honey section cards, calm teal synthesis cards, and high-contrast text.
- **Label explanation:** The design mockup includes a single graph card explaining rail vocabulary such as section labels (`03 Upskill`), state labels (`queued`, `active`), support beat states (`shown`, `tap`), and the final summary card.
- **Motion:** Use one card entrance and one beat reveal transition, gated by `prefers-reduced-motion`. Do not animate text in a way that prevents selection or reading.
...
- U1. Add pure hook presentation helpers.
  ...
  - **Test scenarios:**
    ...
    - Keywords are trimmed, deduplicated, limited, and ordered as beats.
...
### Phase 2. Recall interaction and staged supports

**Goal:** Turn the prototype from a visual skin into an active recall sprint by staging support beats and coordinating with Study Mode reveal behavior.

**Implementation units**

- U3. Add per-card support reveal state.
  ...
  - **Test scenarios:**
    - In Study Mode, support beats start hidden or softened according to the selected behavior.
    - Tapping a card reveals the next beat without revealing the note body.
    - Reveal-all exposes all beats and note-side answers consistently.
...
- U4. Add keyboard and accessibility behavior for hook cards.
  ...
  - **Test scenarios:**
    - Keyboard users can advance beats and reveal answers.
    - Buttons have labels that distinguish reveal-support from reveal-answer.
    - Reduced-motion preferences disable or soften animation.

**Success evaluation**

- **Recall signal:** During a timed sprint, the learner can answer before revealing beats on most cards where the underlying cue is clear.
- **Comprehension signal:** After revealing beats, the learner can explain why at least two beats relate to the hook rather than merely read terms.
...
- U6. Add polished responsive styling.
  ...
  - **Test scenarios:**
    ...
    - Long questions wrap without covering support beats.
...
- U7. Add a lightweight evaluation fixture or internal dogfood checklist.
  ...
  - **Test scenarios:**
    - Checklist covers recall before reveal, beat usefulness, summary-card synthesis, theme safety, and misleading-hook incidents.
...
- **Comprehension threshold:** Users can explain relationships among beats for most sampled cards.
...
- AE3. Given Study Mode is active, when a card has unrevealed support beats, then the hook question remains readable while support terms are hidden, softened, or advanced one beat at a time.
...
| Support beats become passive hints | Start beats hidden/softened in Study Mode and reveal progressively. |
