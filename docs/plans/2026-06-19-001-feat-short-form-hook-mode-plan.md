...
- **Card anatomy:** Each note section gets a small card vertically aligned beside the Markdown body. The card contains the transformed hook title and a lightweight state label only; it should not show section indexes or support beats by default. The original question remains in the presentation model for fidelity checks and accessibility metadata.
...
- **Label explanation:** The design mockup includes a single graph card explaining that vertical alignment maps cards to Markdown sections, `queued`/`active` are review states, support beats are intentionally omitted, and the summary appears below the note.
- **Motion:** Use one subtle card entrance or focus transition, gated by `prefers-reduced-motion`. Do not animate text in a way that prevents selection or reading.
