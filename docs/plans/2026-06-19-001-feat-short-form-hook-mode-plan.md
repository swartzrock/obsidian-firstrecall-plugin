...
The design direction is documented as a self-contained HTML mockup at `docs/designs/short-form-hook-mode.html`, with a screenshot-style SVG artifact at `docs/screenshots/short-form-hook-mode-design.svg`. The visual thesis is warm editorial energy inside Obsidian constraints: a phone-like sprint panel, oversized hook typography, staged support beats, and a teal final-summary card.

- **Entry surface:** Hook Mode appears as an opt-in Study Mode sprint, not as the default Cornell view. The toolbar should describe it as a review sprint so learners understand they are switching energy levels.
- **Card anatomy:** Each section card contains a progress marker, a transformed hook title, staged support beats, and separate actions for revealing a beat versus revealing the answer. The original question remains in the presentation model for fidelity checks and accessibility metadata.
...
- **Final summary card:** The note summary becomes the sprint landing card, with the learning objective as optional quiet supporting text. This card should feel like synthesis, not another hint.
- **Theme translation:** Production CSS should express this through Obsidian variables first (`--background-*`, `--text-*`, `--interactive-accent`) and only use custom fallbacks for the demo page. High-energy color belongs to the opt-in mode root class.
