...
`src/editor-cue-display.ts` (Editing View presentation options), retained `src/cornell-layout.ts`
and `src/cornell-style.ts` (Cornell editor-card sizing and fixed presentation),
`src/settings.ts` (provider and display settings),
